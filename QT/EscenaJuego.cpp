...
{
    // ── Jugador ──────────────────────────────────────────────
    shQuiet .load(":/new/Recursos/jugador/spritejugador0.png");  // 4 frames 637x416
    shIdle  .load(":/new/Recursos/jugador/spritejugador1.png");  // 4 frames 636x416
    shRun1  .load(":/new/Recursos/jugador/spritejugador2.png");  // 7 frames 363x416
    shRun2  .load(":/new/Recursos/jugador/spritejugador3.png");  // 6 frames 344x512
    shThrow .load(":/new/Recursos/jugador/spritejugador4.png");  // 4 frames 448x592
    shJump  .load(":/new/Recursos/jugador/spritejugador5.png");  // 4 frames 516x512
    shCrouch.load(":/new/Recursos/jugador/spritejugador6.png");  // 4 frames 516x512
...
void EscenaJuego::paintEvent(QPaintEvent* event)
{
    ...
    if (menuActivo)      { dibujarMenu(painter);       return; }
    if (victoria)        { dibujarVictoria(painter);   return; }
    if (gameOver)        { dibujarGameOver(painter);   return; }
    if (nivelCompletado) { dibujarTransicion(painter); return; }
...
// ============================================================
//  HELPER: elige el sprite del jugador según estado e input
// ============================================================
QPixmap EscenaJuego::elegirSpriteJugador()
{
    const bool enMovimiento = teclaIzq || teclaDer;
    const bool sinTecla     = !teclaIzq && !teclaDer && !teclaArriba
                          && !teclaAbajo && !teclaEspacio;

    if (jugadorActual->getSaltando()) {
        return frame(shJump,   frameActual % 4, 516, 512);

    } else if (jugadorActual->getAgachado()) {
        return frame(shCrouch, frameActual % 4, 516, 512);

    } else if (teclaEspacio) {
        return frame(shThrow,  frameActual % 4, 448, 592);

    } else if (enMovimiento) {
        if (frameActual % 2 == 0)
            return frame(shRun1, frameActual % 7, 363, 416);
        else
            return frame(shRun2, frameActual % 6, 344, 512);

    } else if (sinTecla) {
        // quieto: 4 frames de 637x416
        return frame(shQuiet, frameActual % 4, 637, 416);

    } else {
        // idle genérico: 4 frames de 636x416
        return frame(shIdle,  frameActual % 4, 636, 416);
    }
}

// ============================================================
//  JUGADOR NIVEL 1  —  con desplazamiento de cámara
// ============================================================
void EscenaJuego::dibujarJugadorNivel1(QPainter& p, Jugador* jugador)
{
    if (!jugador) return;

    QPixmap sprite = elegirSpriteJugador();
...
// ============================================================
//  HUD  —  hud.png: 2064x512 → 4 iconos de 516x512
//    0=corazón  1=balón  2=reloj  3=rayo
// ============================================================
void EscenaJuego::dibujarHUD(QPainter& p, Jugador* jugador, int tiempo, int puntaje)
{
    ...
    // Corazón + barra de vida
    QPixmap icoCorazon = frame(shHud, 0, 516, 512);
    p.drawPixmap(PAD + 4, PAD + 4, escalar(icoCorazon, ICO, ICO));
    dibujarBarraVida(p, PAD + 4 + ICO + SEP, PAD + 14,
                     jugador->getVida(), jugador->getVidaMaxima(), 200);
    ...
    p.drawText(PAD + 4 + ICO + SEP, PAD + ICO + 10 + ICO - 6,
               QString("PUNTOS: %1").arg(puntaje));

    // Reloj + tiempo (solo nivel 2)
